...
import { GetNjayarPrabhaadham } from "../../utils/api";
...
function NjayarPrabhaadham() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      let respi: any = await GetNjayarPrabhaadham();
      if (respi.networkStatus == 7) {
        setData(respi?.data?.njayarprabhaadhams?.data);
        setLoading(false);
      }
    } catch (error) {
      console.log(error);
...
export default NjayarPrabhaadham;
